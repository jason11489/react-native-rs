...
use ark_r1cs_std::{fields::fp::FpVar, prelude::{AllocVar, AllocationMode, EqGadget, Boolean}};
use ark_relations::r1cs::{Namespace, SynthesisError};

use super::{SymmetricEncryptionScheme, Randomness, SymmetricKey, Ciphertext, Plaintext};
use crate::gadget::{
    hashes::{
...
impl<F> SymmetricEncryptionGadget<SymmetricEncryptionScheme<F>, F> for SymmetricEncryptionSchemeGadget<F>
where
    F: PrimeField + Absorb,
{
...
mod tests {
    ...
    use crate::gadget::{
        hashes::mimc7::{parameters, Parameters, constraints::ParametersVar}, 
    ...
    fn test_semmetic_encryption_gadget() {
        ...
        let m_dec = SymmetricEncryptionScheme::<Fr>::decrypt(rc.clone(), key.clone(), ct.clone()).unwrap();
        println!("m: {:?}", m_dec.m);
        ...
        let param_var = ParametersVar::<Fr>::new_constant(
            ark_relations::ns!(cs, "gadget_const"), 
            &rc.clone(),
        ).unwrap();
        let result_var = MyGadget::encrypt(param_var.clone(), randomness_var, key_var, msg_var).unwrap();
